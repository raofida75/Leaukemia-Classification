...
st.title('Leukemia Classification Application')
...
# predict 
def output(prob):
    st.text('Probability that this human cell contains Leukemia is: {}'
            .format((np.squeeze(prob))))
...
try: 
    ...
    image = resize_image(image,49152)
    # load the parameters
    weights = np.load('weights1.npy', allow_pickle=True)[()]
    # forward propagation
